...
const formatMoneyPlain = (v: number | string) => {
  const n = Number(v) || 0
  // Mostrar separador de miles y sin decimales: 1.646.000
...
<template>
  <div class="space-y-5">
    <section class="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 md:p-5 space-y-4">
      <div>
        <h1 class="text-2xl font-semibold text-slate-900">Tarifas de lentes</h1>
        <p class="text-sm text-slate-600 mt-1">Consulta valores y códigos tarifarios de lentes.</p>
      </div>
    ...
    <table class="table-auto w-full border-collapse border border-slate-200">
      ...
      <tbody>
        <tr v-for="item in items" :key="item.id">
          ...
          <td class="border border-slate-200 p-2">{{ formatMoneyPlain(item.valor) }}</td>
